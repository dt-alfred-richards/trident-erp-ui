...
export function ProductDispatch() {
  const { orders, dispatchProducts } = useOrders()
  const [selectedOrderId, setSelectedOrderId] = useState<string>("")
  const [error, setError] = useState<string | null>(null)
  const [successMessage, setSuccessMessage] = useState<string | null>(null)

  // Filter for eligible orders (approved, ready, partial_fulfillment)
  ...
  // Get products eligible for dispatch (ready, partially_ready, partially_dispatched)
  const eligibleProducts =
    selectedOrder?.products.filter(
      (product) =>
        product.status === "ready" || product.status === "partially_ready" || product.status === "partially_dispatched",
    ) || []

  // Handle dispatch
  const handleDispatch = (productId: string, quantity: number) => {
    try {
      setError(null)
      ...
      dispatchProducts(selectedOrderId, productId, quantity)
      setSuccessMessage(`Dispatched ${quantity} units successfully`)
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>Product Dispatch</CardTitle>
        </CardHeader>
        <CardContent>
                ...
                <Table>
                  ...
                  <TableBody>
                    {selectedOrder.products.map((product) => {
                      ...
                      const remaining = allocated - dispatched
                            ...
                            {isEligible && remaining > 0 ? (
                              <div className="flex items-center justify-end gap-2">
                                <Input
                                  ...
                                  min={1}
                                  max={remaining}
                                  id={`dispatch-${product.id}`}
                                />
                                <Button
                                  size="sm"
                                  onClick={() => {
                                    const input = document.getElementById(`dispatch-${product.id}`) as HTMLInputElement
                                    const quantity = Number.parseInt(input.value)
                                    handleDispatch(product.id, quantity)
                                  }}
                                >
